...
require './lib/ruby/client/response/TransactionResponse'
...
class BlockChainApi
  ...
  def getCurrentBlock(timeout = "4000")
    ...
    if statusCode == 200
      return CurrentBlockResponse.new(response['blockNumber'],
  ...
  def getBlockByNumber(blockNumber, timeout = "4000")
    ...
    if statusCode == 200
      return BlockResponse.new(response['previous_hash'],
                               response['data_hash'],
