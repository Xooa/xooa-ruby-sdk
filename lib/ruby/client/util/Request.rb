...
=begin
      unless response.success?
        if response.timed_out?
          raise XooaApiException.new('0', 'Connection Timed Out')
        elsif response.code == 0
          raise XooaApiException.new('0', :errorMessage => response.return_message)
        else
          raise XooaApiException.new(response.code, response.status_message)
        end
      end
=end
      ...
      begin
        ...
        raise XooaApiException.new('0', e.to_s)
      end
