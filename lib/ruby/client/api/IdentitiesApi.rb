...
class IdentitiesApi
  ...
  def currentIdentity(timeout = "4000")
    ...
    if statusCode == 200
      ...
      if attributes.respond_to?("each")
        ...
        attribute = Attr.new(attributes['name'], attributes['value'], attributes['ecert'])
        attributesList.push(attribute)
      end

      return IdentityResponse.new(response['IdentityName'],
                                  response['ApiToken'],
                                  response['Id'],
                                  response['AppId'],
                                  response['Access'],
    ...
    if statusCode == 200
      ...
      if response.respond_to?("each")
        response.each do |resp|
          ...
          identityResponse = IdentityResponse.new(resp['IdentityName'],
                                                  resp['ApiToken'],
                                                  resp['Id'],
                                                  resp['AppId'],
                                                  resp['Access'],
  ...
  def enrollIdentity(identityRequest, timeout = "4000")
    ...
    begin
      request = requestUtil.buildRequest(url, 'POST', :headerParams => headerParams,
                                         :queryParams => queryParams, :body => postBody)
  ...
  def enrollIdentityAsync(identityRequest)
    ...
    if statusCode == 202
      return PendingTransactionResponse.new(response['resultId'], response['resultURL'])
    else
      logger.error response
      raise XooaApiException.new(statusCode, response)
    end
  ...
  def deleteIdentity(identityId, timeout = "4000")
    ...
    begin
      request = requestUtil.buildRequest(url, 'DELETE', :headerParams => headerParams,
                                         :queryParams => queryParams, :body => postBody)
